// Hiển thị danh sách thông báo vào bảng, bỏ qua password
let allStaffArr = [];
let currentPage = 1;
let itemsPerPage = 15;
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        let data;
        const cached = localStorage.getItem('noti_list');
        if (cached) {
            data = JSON.parse(cached);
        } else {
            data = await window.notifiService.getNotifi();
            localStorage.setItem('noti_list', JSON.stringify(data));
        }
        // data là object, mỗi key là 1 thông báo
        allStaffArr = Object.entries(data)
            .filter(([key, noti]) => noti && noti.title)
            .map(([key, noti]) => ({ ...noti, id: key }));
        ...
        if (staffBody) staffBody.innerHTML = '<tr><td colspan="6">Lỗi khi tải dữ liệu thông báo.</td></tr>';
    }
    ...
    if (refreshBtn) {
        refreshBtn.onclick = async function() {
            try {
                localStorage.removeItem('noti_list');
                const data = await window.notifiService.getNotifi();
                localStorage.setItem('noti_list', JSON.stringify(data));
                allStaffArr = Object.entries(data)
                    .filter(([key, noti]) => noti && noti.title)
                    .map(([key, noti]) => ({ ...noti, id: key }));
...
function renderStaffListPaged(page, perPage) {
    ...
    staffBody.innerHTML = pageStaff.map((staff, idx) => `
        <tr>
            <td>${startIdx + idx + 1}</td>
            <td>${staff.title}</td>
            <td>${staff.priority}</td>
            <td>${formatDateVN(staff.date_create)}</td>
            ...
            <td>
                ...
                <a href="#" class="action-link delete" data-id="${staff.id}" style="color: #e31616;">
                    <img src="/svg/icon_action/delete.svg" alt="Xóa" title="Xóa yêu cầu" style="width:20px;height:20px;vertical-align:middle;">
                </a>
            </td>
        </tr>
    `).join('');
